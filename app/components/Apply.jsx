

const Apply = () => {
    return <div>
        <div className=" bg-opacity-10 mt-[120px]">
            <div className="hero lg:h-[700px] p-6 bg-fixed flex justify-around items-center text-black" style={{ backgroundImage: 'url(/school.jpg)' }}>
                <div className="space-y-6 mt-14">
                    <h1 className="text-8xl font-semibold  font-mono">Apply for Admission        </h1>
                    <p className="">BRACU offers 3 semesters a year: Spring, Summer & Fall, and these start in January, May & September respectively. For Pharmacy & LLB: BRACU offers admission for Pharmacy & LLB department twice a year..</p>
                </div>
                <div>
                    <button className="bg-gradient-to-r h-16 w-[200px] from-purple-400 to-blue-500 hover:from-pink-500 hover:to-purple-600 text-white font-bold py-3 px-6 rounded-full shadow-lg transform transition-all duration-500 ease-in-out hover:scale-110 hover:brightness-110 hover:animate-pulse active:animate-bounce">
                        Apply Now
                    </button>
                </div>
            </div>
        </div>
    </div>;
};
export default Apply;